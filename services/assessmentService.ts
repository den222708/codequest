...
function mapAssessment(raw: any): Assessment {
  ...
  return {
    ...
    type: raw.type?.toLowerCase() || 'quiz',
    ...
    settings: typeof raw.settings === 'string' ? (() => { try { return JSON.parse(raw.settings); } catch { return defaultSettings(); } })() : (raw.settings || defaultSettings()),
...
function mapQuestion(q: any): Question {
  return {
    ...
    topic: q.topic || '',
    tags: typeof q.tags === 'string' ? (() => { try { return JSON.parse(q.tags); } catch { return []; } })() : (q.tags || []),
    points: q.points || 100,
    ...
    boilerplateCode: typeof q.boilerplateCode === 'string' ? (() => { try { return JSON.parse(q.boilerplateCode); } catch { return {}; } })() : (q.boilerplateCode || {}),
    solution: q.solution,
    hints: q.hints,
